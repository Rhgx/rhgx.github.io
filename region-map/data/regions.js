/**
 * Roblox Server Region Data
 * Each region has a name, latitude, and longitude.
 * Add or modify regions here to update the application.
 */
export const regions = [
  // North America
  { name: "Seattle, WA", lat: 47.6062, lon: -122.3321 },
  { name: "Los Angeles, CA", lat: 34.0522, lon: -118.2437 },
  { name: "Dallas, TX", lat: 32.7767, lon: -96.797 },
  { name: "Chicago, IL", lat: 41.8781, lon: -87.6298 },
  { name: "Atlanta, GA", lat: 33.749, lon: -84.388 },
  { name: "Miami, FL", lat: 25.7617, lon: -80.1918 },
  { name: "Ashburn, VA", lat: 39.0438, lon: -77.4874 },
  { name: "New York City, NY", lat: 40.7128, lon: -74.006 },

  // Europe
  { name: "London, UK", lat: 51.5074, lon: -0.1278 },
  { name: "Amsterdam, NL", lat: 52.3676, lon: 4.9041 },
  { name: "Lelystad, NL", lat: 52.5185, lon: 5.4714 },
  { name: "Paris, FR", lat: 48.8566, lon: 2.3522 },
  { name: "Frankfurt, DE", lat: 50.1109, lon: 8.6821 },
  { name: "Warsaw, PL", lat: 52.2297, lon: 21.0122 },
  { name: "Dublin, IE", lat: 53.3498, lon: -6.2603 },

  // Asia Pacific
  { name: "Mumbai, IN", lat: 19.076, lon: 72.8777 },
  { name: "Tokyo, JP", lat: 35.6895, lon: 139.6917 },
  { name: "Singapore", lat: 1.3521, lon: 103.8198 },
  { name: "Sydney, AU", lat: -33.8688, lon: 151.2093 },

  // South America
  { name: "São Paulo, BR", lat: -23.5505, lon: -46.6333 },
];