...
// --- Constants ---
const CLASS_ORDER = [
  "Flanker", "Trooper", "Arsonist", "Annihilator", "Brute",
  "Mechanic", "Doctor", "Marksman", "Agent",
];
const SLOT_ORDER = [
  "Primary", "Secondary", "Watches", "Cloak", "Melee", "PDA", "Sapper",
];
const ALL_CLASS_SLOT_ORDER = ["Melee"];
const OFFCLASSES = [
  "Arsonist", "Annihilator", "Brute", "Mechanic", "Marksman", "Agent",
];
const MODE_MAP = {
...
const BANNED_CLASSES_MAP = {
  status4v5: [], status6v6: [], statusHL: [], statusPro: [],
};
// --- End Constants ---
  ...
  try {
   ...
   // const response = await fetch("weapons-whitelist.json"); // old
    ...
    if (response.ok) {
        ...
        console.error(`HTTP error loading whitelist! status: ${response.status}`);
        whitelistData = null;
        if (weaponContentArea) {
             weaponContentArea.innerHTML = createMessageColumn(
                "error-message-container", "error-message", "Failed to load weapon data."
            );
        }
...
function handleWeaponRouteChange() { // Removed silent parameter
  ...
  requestAnimationFrame(() => {
      ...
      } else { // Handles case where data loaded but hash is invalid/missing
          weaponPageTitle.textContent = "Weapon Whitelist";
          weaponContentArea.innerHTML = createMessageColumn(
              "main-page-content-container", "main-page-content",
...
function displayWeaponMode(modeKey, shouldShowOffclassIndicator) {
      ...
      if (classData[slotName] && classData[slotName].length > 0) {
        ...
        slotData.forEach((item) => {
          ...
          if (item.banReason && item.banReason.trim() !== "") {
             ...
             if (lowerStatus === 'banned' || lowerStatus === 'testing' || lowerStatus === 'under-review' || lowerStatus === 'review') {
  ...
  tippy('#whitelist-content [data-tippy-content]', {
    allowHTML: true, placement: 'right', animation: 'fade', theme: 'custom-dark'
  });
}
